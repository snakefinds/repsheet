...
class App(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Google Sheet Scraper (with picks.ly)")
        self.minsize(760, 540)

        self.var_sheet = tk.StringVar()
        self.var_api_key = tk.StringVar()
        self.var_out = tk.StringVar(value=os.path.abspath("sheet_scrape_with_picksly.json"))
    ...
    def _build(self) -> None:
        ...
        header = ttk.Label(
            root,
            text="Scrape a Google Sheet into JSON (adds picks.ly from Kakobuy links).",
        ...
        footer = ttk.Label(
            root,
            text="Tip: picks.ly is derived from the marketplace URL inside kakobuy’s ?url= parameter.",
            foreground="#555555",
        )
    ...
    def _validate(self) -> ScrapeConfig | None:
        ...
        affcode = self.var_affcode.get().strip() or "7hjf5"
